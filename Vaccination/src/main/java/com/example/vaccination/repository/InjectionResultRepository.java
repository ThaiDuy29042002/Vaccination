...
public interface InjectionResultRepository extends JpaRepository<InjectionResult,Integer> {
    ...
    @Query("SELECT ir FROM InjectionResult ir " +
            ...
            "WHERE (:startDate IS NULL OR ir.injectionDate BETWEEN :startDate AND :endDate) " +
            "AND (:vaccineTypeName IS NULL OR vt.vaccineTypeName = :vaccineTypeName) " +
            "AND (:prevention IS NULL OR ir.prevention = :prevention)")
    List<InjectionResult> findResults(
